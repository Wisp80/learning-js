...
найдено, то вернется -1. Есть второй необязательный параметр, который указывает с какой позиции надо начинать 
поиск.*/
let string10 = 'aabbcc';
...
/*Оператор побитовый НЕ "~" преобразует число в 32-разрядное целое со знаком. Дробная часть отбрасывается. Затем 
все биты числа инвертируются. То есть любое число превращается в "-(n+1)".*/
let string13 = 'aac';
...
отсутствует, то поиску будет идти до конца строки. Если использовать отрицательные параметры, то это будет 
означать, что позиция определена как заданное количество символов с конца строки.*/
...
второго. Если первый параметр больше второго, то они сработают как будто их поменяли местами. Но метод 
"substring()" не работает с отрицательными параметрами, они будут считаться как 0.*/
...
/*Строки кодируются в UTF-16. Таким образом, у любого символа есть соответствующий код. Символы сравниваются по 
их кодам. Такой код можно получить при помощи специальных методов.*/
...
/*Для "правильного" сравнения строк внутри разных языков можно использовать метод "localeCompare()". Если 
строка, у которой вызывается метод, меньше строки в параметре, то вернется -1. Если строка, у которой вызывается 
метод, больше строки в параметре, то вернется 1. Если же строки равны, то вернется 0. У этого метода есть 2 
дополнительных параметра. Первый позволяет указать язык (по умолчанию берется из окружения). Второй определяет 
дополнительные правила, такие как чувствительность к регистру, а также следует ли учитывать различия между "a" 
и "á".*/
