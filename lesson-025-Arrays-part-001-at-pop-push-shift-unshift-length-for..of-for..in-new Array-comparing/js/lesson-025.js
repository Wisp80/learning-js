...
/*Массив это подвид объектов, где в квадратных скобках вместо ключа используется индекс элемента. Движок 
JavaScript старается хранить элементы массива в непрерывной области памяти, один за другим. Существуют и другие 
способы оптимизации, благодаря которым массивы работают очень быстро. Но их эффективность падает если использовать 
массивы как объекты.*/
...
/*Методы "push()" и "pop()" выполняются быстро, а методы "shift()" и "unshift()" – медленно. Поскольку методы 
"push()" и "pop()" работают с концом массива, то нет необходимости менять индексы у других элементов, в отличии 
от методов "shift()" и "unshift()", когда происходит смещение всех элементов. Все четыре метода в конце работы 
обновляют свойство "length".*/
...
/*Поскольку массив является объектом, то можно массив перебрать при помощи "for..in". Но на самом деле это 
плохая идея так, как цикл "for..in" выполняет перебор всех свойств объекта, а не только цифровых, что может 
привести к тому, что в некоторых ситуациях "псевдомассивы", то есть объекты, которые выглядят, как массив, будут 
выводить дополнительные нечисловые свойства и методы, которые нам обычно не нужны. Также цикл for..in 
оптимизирован под произвольные объекты, а не массивы, и поэтому этот цикл может работать в 10-100 раз медленнее.*/
...
/*При помощи "new Array()" можно создавать массивы, при этом если указать только один числовой параметр, то 
будет создат пустой массив указанной длинны.*/
