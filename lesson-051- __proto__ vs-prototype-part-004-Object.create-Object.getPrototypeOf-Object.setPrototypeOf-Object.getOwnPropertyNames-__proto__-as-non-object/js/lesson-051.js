...
/*Технически мы можем установить/получить свойство "[[Prototype]]" в любое время. Но обычно мы устанавливаем прототип 
только один раз во время создания объекта, а после уже не меняем. И JavaScript движки хорошо оптимизированы для этого. 
Изменение прототипа "на лету" с помощью метода "Object.setPrototypeOf()" или obj.__proto__= – очень медленная операция, 
которая ломает внутренние оптимизации для операций доступа к свойствам объекта. Так что лучше избегайте этого кроме 
тех случаев, когда вы знаете, что делаете, или же когда скорость JavaScript для вас не имеет никакого значения.*/
...
/*Чтобы изменить указанное поведение, можно создать объект через метод "Object.create()" и в качестве прототипа указать
null. Таким образом не будет унаследованного геттера/сеттера для свойства "__proto__". Теперь это свойство 
