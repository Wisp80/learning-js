...
такие объекты называют "thenable", и этот объект будет обработан как промис. Смысл в том, что сторонние библиотеки 
могут создавать свои собственные совместимые с промисами объекты. Они могут иметь свои наборы методов и при этом быть 
совместимыми со встроенными промисами, так как реализуют метод "then()".*/
...
Метод "fetch()" запрашивает по сети указанный url и возвращает новый промис. После того, как удаленный ресурс присылает 
заголовки ответа, но до того, как весь ответ сервера полностью загружен, этот промис резольвится и в своем скрытом 
свойстве "result" имеет объект "response".
